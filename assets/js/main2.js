...
// _________________________lev 2-6 img random_______________________________
let data = [
    {
        id: "0",
...
let myImgRandom = imgRandom = () => { return Math.floor(Math.random() * data.length) };


console.log(myImgRandom)

let container = document.getElementById('content')
let button = document.createElement('button')
let textButton = document.createTextNode('Click Me')
let myText = document.createTextNode('Zufälliges Bild mit JavaScript anzeigen')
let paragraphs = document.createElement('p')
paragraphs.appendChild(myText)
container.appendChild(paragraphs)
button.appendChild(textButton)
container.appendChild(button)

let myImag = document.createElement('img')
container.appendChild(myImag)
...
function changeImg() {
    myImag.setAttribute('src', data[imgRandom()].download_url)
    myImag.style.width = '360px'
}
